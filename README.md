...
模拟盘执行四层组合：**极端 MR + EMA10/100 日线趋势 + 现货多/永续空资金费
carry + 每日真实执行探针**。各策略先生成目标敞口，再汇总成同一账户的 BTCUSDT
永续净仓位；carry 的现货腿使用 Spot Testnet，资金费以账户实际入账归因。
组合在模拟盘验收前禁止连接主网（详见 `docs/PORTFOLIO_RESEARCH.md`）。
...
- **策略归因**：展示四个 sleeve 的目标仓位、净贡献、实际费用、回合数和胜率
- 交易任务由控制面托管：重复启动返回 409；单策略模式暂停会保留保护单，四层组合模式暂停会撤回受管永续净仓并把 carry 现货恢复到启动基线
